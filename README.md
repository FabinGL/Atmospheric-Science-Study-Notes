...
#### 评分步骤
- 逐站判定预报的趋势是否正确，统计出趋势预测正确的总站数N0；
- 逐站判定一级异常预报是否正确，统计出一级异常预测正确的总站数N1；
- 逐站判定二级异常预报是否正确，统计出二级异常预测正确的总站数N2；
- 没有预报二级异常而实况出现降水距平百分率≥100%或等于-100%、气温距平≥3℃或≤-3℃的站数（称为漏报站，记为M）；
- 统计实际参加评估的站数N，即规定参加考核站数减去实况缺测的站数；
- 使用公式

$$
Ps=100 \times \frac{A*N_0+B*N_1+C*N_2}{N-N_0+A*N_0+B*N_1+C*N_2+M}
$$
